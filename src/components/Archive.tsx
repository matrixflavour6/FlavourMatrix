...
import { Search, Filter, BookOpen, Clock, Lock, Unlock, ChevronDown, ChevronUp, Bookmark, BookmarkCheck, Calendar, FileText } from 'lucide-react';
...
export default function Archive({ subscriber, savedArticleIds, onToggleSaveArticle, onOpenSubscribeModal }: ArchiveProps) {
  ...
  const handleArticleClick = (article: Article) => {
    if (article.isPremium && !subscriber.isSubscribed) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-left border-l-4 border-zinc-900 pl-4 max-w-3xl">
          ...
          <p className="mt-2 text-sm text-zinc-500">
            Browse our laboratory research logs and exclusive community protocols. Enter a chemical compound or food item to locate associated cooking formulas.
          </p>
        </div>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 max-w-5xl mx-auto">
          {filteredArticles.length > 0 ? (
            filteredArticles.map((article) => {
              ...
              const canRead = !article.isPremium || subscriber.isSubscribed;
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6 sm:p-8 flex flex-col md:flex-row gap-6 items-start justify-between text-left">
                    ...
                    <div className="flex-grow flex flex-col gap-2 max-w-3xl">
                      ...
                      <div className="mt-4 flex items-center gap-2">
                        ...
                        {article.isPremium && (
                          <span className={`inline-flex items-center gap-1 rounded-md px-2.5 py-0.5 text-[10px] font-mono font-semibold uppercase tracking-wider ${
                            subscriber.isSubscribed
                              ? 'bg-emerald-50 border border-emerald-100 text-emerald-800'
                              : 'bg-zinc-100 border border-zinc-200 text-zinc-600'
                          }`}>
                            {subscriber.isSubscribed ? <Unlock className="h-2.5 w-2.5" /> : <Lock className="h-2.5 w-2.5" />}
                            {subscriber.isSubscribed ? 'UNLOCKED' : 'MEMBER ONLY'}
                          </span>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                        ...
                        >
                          {isExpanded ? (
                            ...
                            <>
                              <span>{article.isPremium && !subscriber.isSubscribed ? 'Unlock Log' : 'Read Protocol'}</span>
                      ...
                      <div className="prose prose-zinc max-w-4xl mx-auto font-sans leading-relaxed text-zinc-700">
                        ...
                        <div className="whitespace-pre-line text-sm sm:text-base">
                          ...
                          {article.content.split('\n').map((line, idx) => {
                            ...
                            if (line.startsWith('| ')) {
                              // Treat as table parsing helper or custom format
                              return (
                        ...
                        <div className="mt-8 border-t border-zinc-200 pt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                          <div className="flex items-center gap-2 font-mono text-xs text-zinc-500 uppercase">
                            ...
                            <span>STRICT INTELLECTUAL COUPLING. DO NOT REPRODUCE OUTSIDE FLAVOUR MATRIX.</span>
                          </div>
                          ...
                          >
                            ...
                            {isSaved ? 'SAVED TO MY MATRIX' : 'SAVE PROTOCOL'}
                          </button>
                        </div>
                  ...
                  {/* PREVIEW LOCK BLUR SCREEN */}
                  {article.isPremium && !subscriber.isSubscribed && (
                    <div className="hidden border-t border-zinc-100 bg-zinc-50 p-6 text-center">
                      {/* Fallback indicator */}
                      <p className="font-mono text-xs text-zinc-500">Access Restricted. Subscribe to read.</p>
                    </div>
                  )}
